"""Rollout probe for the drift-gated + multistep combo experiment.

Loads checkpoints from `experiments/results/drift_gated_multistep_ckpts/`
(saved by `experiments/drift_gated_multistep_ablation.py`) and rolls
each autoregressively at H ∈ {15, 50, 100}.

Reports per-step MSE / cos_div + per-step mean gate value (so we can
see whether the gate stays open during rollout, the load-bearing
question for the combo).
"""
    ...
    with torch.no_grad():
        for ti in range(n_use):
            ...
            for s in range(max_h):
                # Forward + capture gate
                pred = model(buf)
                # Re-compute gate for telemetry (uses last frame in buf)
                with torch.no_grad():
                    x_t = buf[:, -1]
                    cml_mean = model.rens(x_t).detach()
                    drift = ((x_t - cml_mean) ** 2).mean(dim=1, keepdim=True).sqrt()
                    gate = torch.sigmoid(model.gate_scale * (-drift + model.gate_bias))
                gt_idx = ti * T + K - 1 + s
...
def main():
    out_path = Path("experiments/results/drift_gated_multistep_rollout_probe.json")
    ...
    for bn, gf in BENCHMARKS.items():
        for seed in SEEDS:
            ...
            if not ckpt.exists():
                ...
                continue
            print("=" * 78)
            print(f"probe combo  {bn}  seed={seed}  context_k={CONTEXT_K}")
            print("=" * 78)
            try:
    ...
    print()
    print("=" * 100)
    print("SUMMARY — drift-gated + multistep combo (median across seeds)")
    print("=" * 100)
