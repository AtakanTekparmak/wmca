"""Multi-config CML ablation -- parallel passes with output blending.

Compares:
  - rescor                     (frozen CML, default eps=0.30, beta=0.15)
  - rescor_discrete_global     (softmax over K=5 blended eps/beta -- gradient through chaos)
  - rescor_multi_config        (K=3 parallel CML passes, output blending -- gradient-free CML)

The key difference: rescor_discrete_global blends eps/beta BEFORE the CML pass,
so gradient to the logits still goes through 15 chaotic steps. rescor_multi_config
runs K separate CML passes with fixed configs, detaches their outputs, and blends
the outputs -- gradient to the logits is d(loss)/d(blend) * cml_out_k, with zero
CML interior in the backward pass.

Usage:
    PYTHONUNBUFFERED=1 uv run --with scikit-learn,scipy python experiments/multi_config_ablation.py
"""
import json
import time
...
MODELS = ["rescor", "rescor_discrete_global", "rescor_multi_config"]
...
def main():
    ...
    for model_name in MODELS:
        ...
        for bench_name, gen_fn in BENCHMARKS.items():
            ...
            # Get selection info for gate models
            select_info = ""
            ...
            print(
                f"  {bench_name:15s}  {meta['metric']}={score:.6e}  "
    ...
    out_path = Path("experiments/results/multi_config_ablation.json")
